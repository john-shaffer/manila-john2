(ns manila-john.http-client
  (:require [clj-http.client :as http]
            [cheshire.core :as json]
            [clojure.java.io :as io]
            [clojure.string :as str]
            [manila-john.util :as util])
  (:use [cemerick.uri :only (uri)]
...
(def ^{:private true} user-agent (str "manila-john.http-client"))
(def ^{:dynamic true} *default-data-type* "application/json")
(def ^{:dynamic true} *configuration-defaults*
  {:socket-timeout 0
    :conn-timeout 5000
...
(def  ^{:doc "When thread-bound to any value, will be reset! to the complete
HTTP response of the last couchdb request."
        :dynamic true}
  *response* nil)
...
(defn lazy-view-seq
  "Given the body of a view result or _changes feed (should be an InputStream),
   ...
   header? should be true when the result is expected to include additional
   information in an additional header line, e.g. total_rows, offset, etc. — in
   other words, header? should always be true, unless the response-body is from 
   a continuous _changes feed (which only include data, no header info). This 
   header info is added as metadata to the returned lazy seq."
...
(defn view-request
  "Accepts the same arguments as couchdb-request*, but processes the result
assuming that the requested resource is a view (using lazy-view-seq)."
  [method url & opts]
